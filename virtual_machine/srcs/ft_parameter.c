...
/*   Updated: 2020/03/10 18:45:40 by pauljull         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_get_param_value(t_process *process, t_vm *vm)
{
	...
	while (j < nb_param)
	{
		if (vm->param[j][1] == REG_BIT)
			...
			ft_convert_param(vm, i + tab_instruction[process->opcode - 1].dir_size, &i, j);
		j += 1;
	}
...
int	ft_bad_ocp_parsing(unsigned char ocp, t_process *process)
{
	...
	while (j < nb_param)
	{
		...
		if (option == REG_BIT)
		{
			if (tab_instruction[process->opcode - 1].param_type[j] & T_REG)
				process->pc += 1;
		}
		else if (option == IND_BIT)
		{
			if (tab_instruction[process->opcode - 1].param_type[j] & T_IND)
				process->pc += 2;
		}
		else if (option == T_DIR)
		{
			if (tab_instruction[process->opcode - 1].param_type[j] & T_DIR)
				process->pc += tab_instruction[process->opcode - 1].dir_size;
		}
		else
		{
			process->pc += 1;
